...
namespace OrderManageSystem
{
    ...
    {
         void InputOrderItemIO(int i, OrderItem orderItem);//添加item的交互
         void InputOrderIO(Order order);//添加order的交互
         long DeleteOrderIn();//删除order的输入
         long ModifyOrderByNumIn();//修改order的输入
         string ShowAllOrdersOut(List<Order> orderlList);//展示所有order
         void ShowOrderOut(Order order, ShowWay showWay, int n = 0);//展示某个order
         SortWay SortBySomeWayIO();//输入搜索方式
         SearchWay SearchBySomeWayIO();//搜索order
         long SearchByOrderNumIO();//通过订单号搜索
         string SearchByCustomerNameIO();//通过用户名搜索
         string GetImportFileName();//获取导入文件名
         string GetExportFileName();//获取导出文件名
         void StartMenu();//开始菜单
    }
    ...
    public class OrderService //订单服务
    {
        ...
        public OrderService()
        {
            ...
            //serviceIo=new CMDServiceIO();//命令行交互接口
            serviceIo = new WinFormServiceIO();//WinForm交互接口
        ...
        public string ShowAllOrders()//显示所有的Order
        {
            return serviceIo.ShowAllOrdersOut(orderList);
        }
